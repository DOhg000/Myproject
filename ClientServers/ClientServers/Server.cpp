...
DWORD WINAPI serverReceive(LPVOID lpParam) { //Получение данных от клиента
	...
	while (true) { //Цикл работы сервера
		if (recv(client, buffer, sizeof(buffer), 0) == SOCKET_ERROR) {
			...
			cout << "Ошибка получения данных буфера, или же клиент отсоединился, код ошибки: " << WSAGetLastError() << endl;
			return -1;
		}
		if (strcmp(buffer, "exit\n") == 0) { //Если клиент отсоединился
			cout << "Потеря соединения с клиентом." << endl;
			break;
		}
...
int main()
{
	...
	if (bind(server, (SOCKADDR*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
		cout << "Не удалось привязать данные к сокету, код ошибки: " << WSAGetLastError() << endl;
		return -1;
	}

	if (listen(server, 0) == SOCKET_ERROR) { //Если не удалось получить запрос
		cout << "Не удалось прослушать запросы, код ошибки: " << WSAGetLastError() << endl;
		return -1;
	}
	cout << "Прослушивание подключений..." << endl;
	...
	if ((client = accept(server, (SOCKADDR*)&clientAddr, &clientAddrSize)) != INVALID_SOCKET) {
		...
		if (closesocket(server) == SOCKET_ERROR) { //Ошибка закрытия сокета
			cout << "Ошибка закрытия сокета, код ошибки:  " << WSAGetLastError() << endl;
			return -1;
		}
		WSACleanup();
	}
